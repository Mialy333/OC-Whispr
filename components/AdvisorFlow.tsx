...
export default function AdvisorFlow({ fid, onBack }: Props) {
  ...
  const selBtn = (selected: boolean, green = false): React.CSSProperties => ({
    ...
    background:   selected
      ? (green ? 'rgba(42,168,75,0.12)' : 'rgba(62,111,168,0.12)')
      : 'var(--bg-primary)',
    borderRadius: 10,
    cursor:       'pointer',
    transition:   'all .15s',
    ...
    return (
      <div style={{ padding: '14px 14px 24px', background: 'var(--bg-primary)', minHeight: '100%' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
          ...
          <span style={{ ...mono, fontSize: 9, color: 'var(--text-muted)', letterSpacing: 1.5 }}>YIELD STRATEGY</span>
        </div>
            ...
            <div key={i} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 5, flexShrink: 0, paddingTop: 2 }}>
                  <span style={{ ...mono, fontSize: 10, fontWeight: 700, color: SA.phosphorGlow, border: `1px solid ${SA.phosphorGlow}`, padding: '2px 7px', borderRadius: 4 }}>
                    {advice.estimatedApy}
                  </span>
              ...
              <div style={{ ...mono, fontSize: 11, color: 'var(--text-primary)', marginBottom: 6, lineHeight: 1.5, fontWeight: 600 }}>
                {advice.strategy}
              </div>
              <div style={{ ...serif, fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.55, marginBottom: 10 }}>
                {advice.rationale}
              </div>

              <div style={{ borderTop: '0.5px solid var(--border)', paddingTop: 9, display: 'flex', gap: 7, alignItems: 'flex-start' }}>
                <span style={{ ...mono, fontSize: 9, color: SA.aqua, fontWeight: 700, flexShrink: 0, marginTop: 1, letterSpacing: 0.5 }}>NEXT</span>
                <span style={{ ...mono, fontSize: 11, color: 'var(--text-primary)', lineHeight: 1.45 }}>{advice.actionStep}</span>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ padding: '14px 14px 24px', background: 'var(--bg-primary)', minHeight: '100%' }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {[1, 2, 3].map((n) => (
            <div key={n} style={{
              ...
              background: n === step ? SA.aqua : n < step ? SA.phosphorGlow : 'var(--border)',
              transition: 'all .2s',
            }} />
          ))}
        </div>
        <span style={{ ...mono, fontSize: 9, color: 'var(--text-muted)', letterSpacing: 1 }}>
          {step}/3
        </span>
      </div>
      ...
      {step === 1 && (
        <>
          <h2 style={{ ...serif, fontSize: 28, fontWeight: 400, color: 'var(--text-primary)', margin: '0 0 18px', lineHeight: 1.05, letterSpacing: -0.5 }}>
            What&apos;s your style?
          </h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {RISK_OPTS.map(({ label, value, desc }) => (
              <button key={value} onClick={() => setRisk(value)}
                ...
                <div style={{ ...serif, fontSize: 17, color: risk === value ? SA.aqua : 'var(--text-primary)', marginBottom: 2 }}>{label}</div>
                <div style={{ ...mono, fontSize: 10, color: 'var(--text-muted)', lineHeight: 1.4 }}>{desc}</div>
              </button>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {CAPITAL_OPTS.map(({ label, value }) => (
              <button key={value} onClick={() => setCapital(value)}
                ...
                <div style={{ ...serif, fontSize: 22, color: capital === value ? SA.aqua : 'var(--text-primary)' }}>{label}</div>
              </button>
            ))}
          </div>
      ...
      {step === 3 && (
        <>
          <h2 style={{ ...serif, fontSize: 28, fontWeight: 400, color: 'var(--text-primary)', margin: '0 0 6px', lineHeight: 1.05, letterSpacing: -0.5 }}>
            What interests you?
          </h2>
          <p style={{ ...mono, fontSize: 9, color: 'var(--text-muted)', letterSpacing: 1, margin: '0 0 16px' }}>
            ...
            {ASSET_OPTS.map(({ label, value }) => {
              ...
              return (
                <button key={value} onClick={() => toggleAsset(value)}
                  style={{ ...selBtn(sel, true), padding: '15px 10px', textAlign: 'center' }}>
                  <div style={{ ...mono, fontSize: 12, fontWeight: 700, color: sel ? SA.phosphorGlow : 'var(--text-primary)' }}>
                    {label}
                  </div>
                </button>
              );
